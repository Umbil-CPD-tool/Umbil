  ...
  return `
    <html>
      ...
      <body>
        ...
        <div class="print-section"><div class="section-title">Consultations</div><table>${appointmentRows}</table></div>
        ${reflectionHtml}
        ${commentsHtml ? `<div class="print-section"><div class="section-title">Patient Comments</div>${commentsHtml}</div>` : ""}
        ${customHtml ? `<div class="print-section"><div class="section-title">Practice-Specific Questions</div>${customHtml}</div>` : ""}
      </body>
    </html>
  `;
};
...
const PsqDetailScreen = () => {
  ...
  const applyPack = (text: string) => {
    setPackText(text);
    setReflection(reflectionBodyFromPack(parseAppraisalPack(text), "patients"));
  };
  ...
  useEffect(() => {
    void getPsqSurvey(id).then(({ data }) => {
      ...
      if (data?.executive_summary) applyPack(data.executive_summary);
      setLoading(false);
    });
    void getMyProfile()
    ...
    try {
      ...
      await streamReflection({
        body: {
          ...
          comments: statsData.textFeedback.map((t) => t.good).filter(Boolean).slice(0, 24),
          improveComments: statsData.textFeedback.map((t) => t.improve).filter(Boolean).slice(0, 24),
        },
        onChunk: (text) => {
          fullPack += text;
      ...
      applyPack(fullPack);
      await updatePsqExecutiveSummary(id, fullPack);
    } catch {
      setPackText(
        "Unable to generate automatic appraisal summary at this time. Please review the detailed metrics below."
      );
    } finally {
  ...
  const exportPdf = async () => {
    ...
    try {
      const html = buildPsqReportHtml(survey, analytics, packText, reflection);
          ...
          {!isThresholdMet ? (
            ...
            <>
              <View style={styles.card}>
                ...
                {isGeneratingPack && !executiveSummary ? (
                  <Text style={styles.cardHintPulsing}>
                    Analysing responses into appraisal evidence…
                  </Text>
                ) : (
                ...
                <View style={styles.themeRow}>
                  <View style={[styles.card, styles.themeCard]}>
                    ...
                    {pack.strengths.length > 0 ? (
                      ...
                      <Text style={styles.cardHint}>{isGeneratingPack ? "Extracting themes…" : "No themes yet."}</Text>
                    )}
                  </View>
                  <View style={[styles.card, styles.themeCard]}>
              ...
              <View style={styles.card}>
                ...
                <Pressable
                  style={[styles.outlineBtnFull, isGeneratingPack && { opacity: 0.6 }]}
                ...
                <TextInput
                  ...
                  placeholder="Tap Auto-Draft for What patients valued / Surprised / Continue / Improve / Measure / PDP…"
                  placeholderTextColor={colors.textMuted}
                />
