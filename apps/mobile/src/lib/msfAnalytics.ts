// Client-portable MSF analytics — mirrors web `src/lib/msf-analytics.ts` exactly so the mobile
// results screen produces identical GMC domain averages/stats from the same raw `msf_responses` rows.
import { MSF_QUESTIONS, type MsfDomain } from "./msfQuestions";
...
export function calculateMsfAnalytics(
  cycle: any,
  responses: any[]
): MsfAnalyticsResult {
  ...
  const uniqueDomains = Array.from(new Set(MSF_QUESTIONS.map((q) => q.domain as MsfDomain)));
  ...
  const breakdown = Object.entries(domainScores)
    .map(([name, data]) => ({
      id: name,
      name,
      score: data.count > 0 ? parseFloat((data.sum / data.count).toFixed(2)) : "Insufficient Data",
      count: data.count,
    }))
    .sort((a, b) => {
  ...
  const averageScore = totalScoreCount > 0 ? parseFloat((totalScoreSum / totalScoreCount).toFixed(2)) : 0;
