...
function AnalyticsContent() {
  ...
  // FIXED: Depend on totalResponses to ensure we have fresh data, and pass data explicitly.
  useEffect(() => {
    if (data.stats.thresholdMet && !reflection && !isGenerating && !hasAutoGenerated.current && data.stats.totalResponses > 0) {
        hasAutoGenerated.current = true;
        handleGenerateReflection(data);
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [data.stats.thresholdMet, data.stats.totalResponses]);
  ...
  const fetchStats = async () => {
    try {
        ...
        let query = supabase
          .from('psq_surveys')
          .select('id, title, created_at, required_responses, psq_responses(id, answers, created_at)')
          .eq('user_id', user.id);
  ...
  // FIXED: Accepts currentData as an argument to avoid React stale closures
  const handleGenerateReflection = async (currentData = data) => {
    if (currentData.stats.totalResponses === 0) return;
    ...
    try {
        ...
        const response = await fetch('/api/generate-reflection', {
            ...
            body: JSON.stringify({
                ...
                strengths: currentData.stats.topArea,
                weaknesses: currentData.stats.lowestArea,
                comments: currentData.textFeedback.slice(0, 5).map(t => t.good || t.improve).filter(Boolean)
            })
        });
        ...
        if (!response.body) throw new Error("No stream returned from API.");
        ...
        console.error(e);
        alert(e.message || "Failed to generate reflection. Please try again.");
    } finally {
  ...
  const printReport = () => {
    ...
    const reflectionHtml = reflection 
        ...
        : `<div class="no-print" style="background: #f8fafc; border: 1px dashed #cbd5e1; padding: 15px; text-align: center; font-style: italic; color: #64748b; margin-bottom: 30px; border-radius: 8px;">Tip: Please wait for your AI reflection to finish generating before printing to include it in your portfolio.</div>`;
      ...
      <html>
        <head>
          ...
          <style>
            ...
            @media print { 
                ...
                .dashboard, .reflection-box, .comment-box { break-inside: avoid; }
        ...
        <body>
          ...
          <div class="dashboard">
             ...
             <div class="stat-box"><span class="stat-val" style="font-size: 18px; line-height: 1.4;">${formatDomainName(data.stats.topArea)}</span><span class="stat-label">Highest Rated Area</span></div>
          </div>
          ${reflectionHtml}
    ...
    <section className="bg-[var(--umbil-bg)] min-h-screen font-sans text-[var(--umbil-text)]">
      <div className="container mx-auto max-w-[900px] px-5 py-8 pb-20">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
            ...
            <StatCard label="Key Strength" value={formatDomainName(data.stats.topArea)} sub="Highest Domain" icon={<Award size={20} />} isText />
        </div>
        ...
        <div className={`${cardClass} mb-8 p-1`}>
            <div className="bg-[var(--umbil-hover-bg)]/50 p-6 border-b border-[var(--umbil-card-border)] flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
                ...
                <button 
                    onClick={() => handleGenerateReflection(data)}
            ...
            <div className="p-6 relative group bg-[var(--umbil-surface)] min-h-[300px]">
                 {isGenerating && !reflection ? (
                    <div className="w-full h-full min-h-[300px] flex flex-col items-center justify-center text-[var(--umbil-brand-teal)] opacity-60">
